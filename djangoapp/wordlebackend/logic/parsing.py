...
def get_base_context(request):
    ...
    if not target_data:
        return None, None, JsonResponse({"error": "Server error: No dictionary loaded"}, status=500)
...
def parse_game_request(request):
    ...
    if hard_mode and len(prev_guess) not in [0, 5]:
        return None, JsonResponse({"error": "Wrong length of previous guess"}, status=400)
